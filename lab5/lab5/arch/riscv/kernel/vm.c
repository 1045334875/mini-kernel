    ...
    for(va_temp=va,pa_temp=pa; va_temp < (uint64)va+sz; va_temp += PGSIZE, pa_temp += PGSIZE){
        ...
        for(int level = 2; level > 0 ; level--){
            if(level==2){
                ...
                //if(sz != 0x7dfc000) printk("level: %d addr = %lx data:%lx\n",level,cur_page_addr,*cur_page_addr);
            }
            else if(level==1){
                //VPN[1]->PPN[0]
            ...
            if(((*cur_pte_addr)&1)){//valid, then find next page table entry
                ...
                //if(sz != 0x7dfc000) printk("new malloc:&%lx\n",cur_page_addr);
                ...
                //if(sz != 0x7dfc000)  printk("&%lx = %lx\n",cur_pte_addr, *cur_pte_addr);
                //malloc new page and set entry
            }
            //printk("level-for va:%lx  level:%lx\n",va_temp, level);
        }
        //if(sz != 0x7dfc000) printk("Accomplish mapping   sz = %lx  va = %lx  ->  pa = %lx\n", sz, va_temp, pa_temp);
        //VPN[0]
